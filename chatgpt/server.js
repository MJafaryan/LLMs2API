...
async function generate(prompt, modelId = "gpt-5.1", onChunk = null) {
    ...
    try {
            ...
            const parseSSEData = (text, req) => {
                ...
                for (const line of lines) {
                    ...
                    try {
                        ...
                        if (
                            data.o === "append" &&
                            data.p?.includes("/message/content/parts") &&
            ...
            if (!window.__chatgptIntercepted) {
                ...
                window.fetch = async function (...args) {
                    ...
                    if (activeReqId && window.__chatgptRequests[activeReqId]) {
                        window.__chatgptRequests[activeReqId].debug.push(
                ...
                if (OriginalEventSource) {
                    window.EventSource = function (url, config) {
                        ...
                        if (activeReqId) {
                            const req = window.__chatgptRequests[activeReqId];
                            req.debug.push(
                                `EventSource: ${url.substring(0, 80)}`,
                            );
        ...
        const sendButton = await page.$(
            '[data-testid="send-button"], button[aria-label*="Send"], button[aria-label*="Envoyer"]',
        );
        if (sendButton) {
        ...
        if (!fullResponse?.trim()) {
            ...
            fullResponse = await page.evaluate(() => {
                ...
                const messages = document.querySelectorAll(
                    '[data-message-author-role="assistant"], [class*="assistant"], [class*="markdown"], .agent-turn',
                );
