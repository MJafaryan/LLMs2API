        ...
        for (const line of lines) {
          ...
          try {
            ...
            if (data.v && typeof data.v === 'string' && !data.p && !data.o) {
              if (req.currentFragmentIndex === req.responseFragmentIndex && req.responseFragmentIndex >= 0) {
            ...
            if (data.o === 'APPEND' && data.p && typeof data.v === 'string') {
              ...
              if (match) {
                ...
                if (fragIdx === req.responseFragmentIndex) req.chunks.push(data.v);
              }
            }
...
const MODELS = [
  ...
  { id: 'deepseek-v3.2-thinking-search', name: 'DeepSeek V3.2 (Thinking + Search)' },
];
...
async function handleRequest(req, res) {
  ...
  if (pathname === '/v1/chat/completions' && req.method === 'POST') {
    ...
    try {
      ...
      const { model = 'deepseek-v3.2', messages = [], stream = false } = body;
