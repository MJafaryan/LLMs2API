...
async function cleanupIdlePages() {
    ...
    for (const p of toRemove) {
        ...
        try {
            ...
            pagePool = pagePool.filter((x) => x.id !== p.id);
            log("WARN", `Page ${p.id} cleanup error, removed from pool`, {
                error: e.message,
            });
        }
    }
    ...
    if (pagePool.length === 0) {
        ...
        await newPage.page.goto(TARGET_URL, {
            waitUntil: "domcontentloaded",
            timeout: 120000,
        });
        newPage.busy = false;
...
async function getAvailablePage() {
    ...
    return new Promise((resolve) => {
        const check = setInterval(async () => {
            ...
            const validCount = (
                await Promise.all(pagePool.map((p) => isPageValid(p)))
...
async function selectModel(page, modelId) {
    try {
        ...
        const modelSelector = await page.$(
            'button:has-text("Rapide"), button:has-text("Raisonnement"), button:has-text("Pro"), [class*="model-selector"]',
        );
        if (modelSelector) {
            ...
            for (const opt of options) {
                ...
                if (text?.includes(targetModel)) {
                    ...
                    log(
                        "INFO",
                        `Selected model via partial match: ${targetModel}`,
                    );
                    return true;
                }
            }
...
async function enableImageGeneration(page) {
    try {
        ...
        const outilsBtn = await page.$(
            'button:has-text("Outils"), [aria-label*="Outils"]',
        );
        if (outilsBtn) {
...
async function generate(prompt, modelId = "gemini-preview", onChunk = null) {
    ...
    try {
            ...
            if (!window.__geminiIntercepted) {
                ...
                XMLHttpRequest.prototype.send = function (body) {
                    ...
                    if (activeReqId && window.__geminiRequests[activeReqId]) {
                        window.__geminiRequests[activeReqId].debug.push(
                    ...
                    if (url.includes("StreamGenerate")) {
                        if (activeReqId) {
                            ...
                            this.addEventListener("progress", () => {
                                const newText = this.responseText.substring(
                                    req.lastProcessedLength,
                                );
                                req.lastProcessedLength =
        ...
        let inputArea = await page.$(
            'textarea[placeholder*="prompt"], textarea[placeholder*="Gemini"]',
        );
        if (!inputArea) {
        ...
        const sendButton = await page.$(
            '[aria-label*="Envoyer"], [aria-label*="Send"], button[type="submit"]',
        );
        if (sendButton) {
        ...
        if (isImageGen) {
            ...
            while (Date.now() - startTime < timeout) {
                ...
                if (imageUrl) {
                    log(
                        "INFO",
                        `Image generated: ${imageUrl.substring(0, 100)}...`,
                    );
                    break;
                }
            ...
            try {
                ...
                if (response.ok()) {
                    ...
                    log(
                        "INFO",
                        `Image downloaded via context: ${buffer.length} bytes, ${contentType}`,
                    );
                }
            } catch (e) {
                ...
                try {
                    ...
                    if (imgElement) {
                        ...
                        if (box && box.width > 100 && box.height > 100) {
                            ...
                            log(
                                "INFO",
                                `Screenshot captured: ${box.width}x${box.height}, ${screenshotBuffer.length} bytes`,
                            );
                        }
                    }
                } catch (e) {
            ...
            while (Date.now() - startTime < timeout) {
                ...
                if (result.latestText.length > lastTextLength) {
                    const newContent =
                        result.latestText.substring(lastTextLength);
    ...
    if (pathname === "/v1/images/generations" && req.method === "POST") {
        ...
        try {
            ...
            if (!model.startsWith("nano-banana")) {
                ...
                imageModel = model.includes("pro")
                    ? "nano-banana-pro"
                    : "nano-banana";
            }

            const nanoBananaType = imageModel.includes("pro")
                ? "Nano Banana Pro"
                : "Nano Banana";
            log("INFO", "Image generation request", {
                prompt: prompt.substring(0, 50),
                model: imageModel,
                nanoBanana: nanoBananaType,
            });
...
async function main() {
    ...
    server.listen(PORT, () => {
        ...
        log(
            "INFO",
            "  POST /v1/images/generations - Image generation (Nano Banana)",
        );
        log("INFO", "");
        log("INFO", "Models:");
        log(
            "INFO",
            "  gemini-preview (Rapide), gemini-reasoning (Raisonnement), gemini-pro (Pro)",
        );
        log(
            "INFO",
            "  nano-banana (Preview + Image), nano-banana-pro (Pro + Image)",
        );
    });
