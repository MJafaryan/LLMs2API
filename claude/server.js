/**
 * Claude API Server - Stateless with Parallel Support
 * OpenAI-compatible endpoint for Claude chat
 * Each request = new Claude chat (no session/context)
 */
...
const AUTH_TOKEN = process.env.AUTH_TOKEN || 'sk-claude';
...
async function selectModel(page, modelId) {
  try {
    ...
    const targetModel = modelMap[modelId] || 'Sonnet 4.5';
    
    // Click on model selector dropdown (bottom right, shows current model name)
    const modelSelector = await page.$('button:has-text("Sonnet"), button:has-text("Haiku"), [class*="model-selector"], [class*="model-picker"]');
    if (modelSelector) {
    ...
    // Alternative: try clicking by text directly
    try {
      await page.click(`button:has-text("${targetModel}")`);
      await sleep(300, 500);
      log('INFO', `Selected model via text: ${targetModel}`);
      return true;
    } catch {}
...
// ==================== Claude Generate ====================
async function generate(prompt, modelId = 'claude-sonnet-4.5', onChunk = null) {
  ...
  try {
    ...
    // Inject interceptors for SSE capture (fetch and XHR)
    await page.evaluate((reqId) => {
      if (!window.__claudeRequests) {
        window.__claudeRequests = {};
      }
      
      window.__claudeRequests[reqId] = {
        chunks: [],
        complete: false,
        error: null,
        active: true,
      ...
      // Helper to parse Claude SSE data
        ...
        for (const line of lines) {
          ...
          try {
            ...
            if (data.type === 'content_block_delta' && data.delta?.type === 'text_delta' && data.delta?.text) {
              req.chunks.push(data.delta.text);
            }
      ...
      if (!window.__claudeIntercepted) {
        window.__claudeIntercepted = true;
        
        // Only intercept fetch (Claude uses fetch for SSE)
        const originalFetch = window.fetch;
        window.fetch = async function(...args) {
          const response = await originalFetch.apply(this, args);
          const url = args[0]?.toString?.() || args[0]?.url || '';
          
          const activeReqId = Object.keys(window.__claudeRequests || {})
            .find(id => window.__claudeRequests[id]?.active);
          
          if (activeReqId && window.__claudeRequests[activeReqId]) {
            window.__claudeRequests[activeReqId].debug.push(`fetch: ${url.substring(0, 80)}`);
          }
          
          // Only intercept completion endpoint
          if (url.includes('/completion')) {
            const clone = response.clone();
            const reader = clone.body?.getReader();
            
            if (reader && activeReqId) {
              const req = window.__claudeRequests[activeReqId];
    ...
    // Wait for input - Claude uses contenteditable div
    await page.waitForSelector('[contenteditable="true"], textarea', { timeout: 60000 });
    await sleep(500, 1000);
    
    // Select model if not default (Sonnet)
    if (modelId !== 'claude-sonnet-4.5') {
      await selectModel(page, modelId);
    }
    
    // Find and click the input area
    const inputArea = await page.$('[contenteditable="true"]') || await page.$('textarea');
    if (!inputArea) {
    ...
    while (Date.now() - startTime < timeout) {
      ...
      const result = await page.evaluate((reqId) => {
        const req = window.__claudeRequests?.[reqId];
    ...
    // Cleanup request
    await page.evaluate((reqId) => {
      if (window.__claudeRequests?.[reqId]) {
        window.__claudeRequests[reqId].active = false;
        setTimeout(() => {
          delete window.__claudeRequests[reqId];
        }, 5000);
      }
    }, requestId);
    ...
    if (!fullResponse?.trim()) {
      ...
      fullResponse = await page.evaluate(() => {
        ...
        const messages = document.querySelectorAll('[data-message-author-role="assistant"], [class*="assistant"], [class*="response"]');
...
async function handleRequest(req, res) {
  ...
  if (pathname === '/v1/chat/completions' && req.method === 'POST') {
    ...
    try {
      ...
      const { model = 'claude-sonnet-4.5', messages = [], stream = false } = body;
...
async function main() {
  ...
  server.listen(PORT, () => {
    log('INFO', `Claude Server running on port ${PORT}`);
