...
async function selectModel(page, modelId) {
    try {
        ...
        const modelSelector = await page.$(
            '[class*="model-select"], [class*="model-picker"], button:has-text("Qwen3")',
        );
        if (modelSelector) {
...
async function generate(prompt, modelId = "qwen3-max", onChunk = null) {
    ...
    try {
                            ...
                            if (activeReqId) {
                                (async () => {
                                    try {
                                        ...
                                        if (
                                            window.__qwenRequests[activeReqId]
                                        ) {
                                            window.__qwenRequests[
                                                activeReqId
                                            ].error = e.message;
        ...
        if (isImageGen) {
            ...
            const clicked = await page.evaluate(() => {
                const buttons = document.querySelectorAll(
                    'button, [role="button"], div[class*="chip"], span[class*="chip"]',
                );
            ...
            if (clicked) {
                ...
                try {
                    ...
                    log(
                        "INFO",
                        "Clicked image generation button via text selector",
                    );
                } catch (e) {
                    log("WARN", "Could not find image generation button", {
                        error: e.message,
                    });
                }
            }
            ...
            if (thinking || search) {
                const thinkingClicked = await toggleButton(
                    page,
                    "Pensée",
                    thinking,
                );
                await sleep(200, 400);
                const searchClicked = await toggleButton(
                    page,
                    "Recherche",
                    search,
                );
        ...
        let targetTextarea = await page.$(
            'textarea[placeholder*="Comment"], textarea[placeholder*="aider"], textarea[placeholder*="help"], textarea[placeholder*="message"]',
        );
